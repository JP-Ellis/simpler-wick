...
%L. Bottom Contraction ADDITION
% open or close the bottom contraction 
\def\swick@sbcon{
	\@ifnextchar0{
		\swick@ignore
	}{
...
\def\swick@end#1#2{
  ...
  % L. added contraction width option, also adjusted the start of the contraction line to be a bit lower, 
  % while initially fine, it looked a bit off for the bottom contraction so I altered this to be consistent
    \draw[line width=\swick@lwidth] ($(swick-open#1.north) + (0, 2pt)$) 
          -- ($(swick-open#1.base) + (0, \swick@offset) + #1*(0, \swick@sep)$) 
          -- ($(swick-close#1.base) + (0, \swick@offset) + #1*(0, \swick@sep)$) 
          -- ($(swick-close#1.north) + (0, 2pt)$);
  \tikzexternalenable}

%L. open a bottom contraction 
\def\swick@bcend#1#2{
	\swick@setfalse@#1
	\tikzexternaldisable
	\tikzpicture[remember picture, baseline=(swick-close#1.base)]
	...
	\tikz[remember picture, overlay]
  % 
    \draw[line width=\swick@lwidth] ($(swick-open#1.base) + (0, -2pt)$) 
      -- ($(swick-open#1.north) - (0, \swick@offset) + -#1*(0, \swick@sep)$) 
      -- ($(swick-close#1.north) - (0, \swick@offset) + -#1*(0, \swick@sep)$) 
      -- ($(swick-close#1.base) + (0, -2pt)$);
    \tikzexternalenable}
